...
static char const * const kEmptyDataSetFlipWithRTL =       "emptyDataSetFlipWithRTL";
...
- (NSNumber *)emptyDataSetFlipWithRTL
{
    DZNWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetFlipWithRTL);
    return container.weakObject;
}
...
- (DZNEmptyDataSetView *)emptyDataSetView
{
    ...
    if (!view)
    {
        ...
        if ([[UIApplication sharedApplication] userInterfaceLayoutDirection] == UIUserInterfaceLayoutDirectionRightToLeft && [self.emptyDataSetFlipWithRTL isEqual:@YES]) {
...
-(void)setEmptyDataSetFlipWithRTL:(NSNumber *)emptyDataSetFlipWithRTL {
    objc_setAssociatedObject(self, kEmptyDataSetFlipWithRTL, [[DZNWeakObjectContainer alloc] initWithWeakObject:emptyDataSetFlipWithRTL], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
